# PR: Add ResumeAI to awesome-ai-tools — AI career automation

## Summary

Submitting **ResumeAI** (https://resumeai-bot.ru) for inclusion in the Job Search / Career Automation category.

## Tool details

**ResumeAI** is an AI-powered Telegram bot and web dashboard that automates the job search process end-to-end.

**Core AI features:**
- GPT-4 based resume tailoring — reads the job description, extracts requirements, and writes a targeted resume in the candidate's voice
- PDF generation with clean formatting (reportlab)
- Cover letter generation (optional)
- Interview prep Q&A based on the specific role

**Automation features:**
- Scrapes hh.ru and SuperJob via official APIs
- Applies to matching vacancies automatically
- LinkedIn Easy Apply automation via Playwright
- Configurable filters: salary range, keywords, blacklist companies, experience level

**Tech stack:** Python 3.11, aiogram 3, FastAPI, OpenAI API, Playwright, SQLite, systemd

**Links:**
- Bot: https://t.me/topbestworkerbot
- Web app: https://resumeai-bot.ru/app
- Category: Career Tools / AI Productivity

## README addition (paste into Job Search / Career section)

```markdown
- [ResumeAI](https://resumeai-bot.ru) - AI Telegram bot that writes tailored resumes per vacancy and auto-applies to jobs on hh.ru, LinkedIn, Indeed. [@topbestworkerbot](https://t.me/topbestworkerbot)
```
